...
import com.hung.le.session.ChatSession;
import com.hung.le.util.ChatMessageCodec;
...
@ServerEndpoint(value = "/chat/{sessionId}",
        ...
        configurator = ChatEndpointListener.EndpointConfigurator.class)
@WebListener
public class ChatEndpointListener implements HttpSessionListener
{
    ...
    public void onOpen(Session session, @PathParam("sessionId") long sessionId)
    {
        ...
        HttpSession httpSession = (HttpSession)session.getUserProperties()
                .get(ChatEndpointListener.HTTP_SESSION_PROPERTY);
        try
        {
            ...
            if(sessionId < 1)
            {
                ...
                synchronized(ChatEndpointListener.sessionIdSequenceLock)
                {
                    chatSession.setSessionId(ChatEndpointListener.sessionIdSequence++);
                }
                chatSession.setCustomer(session);
                chatSession.setCustomerUsername(username);
                chatSession.setCreationMessage(message);
                ChatEndpointListener.pendingSessions.add(chatSession);
                ChatEndpointListener.chatSessions.put(chatSession.getSessionId(),
                        chatSession);
            }
            else
            {
                ...
                chatSession = ChatEndpointListener.chatSessions.get(sessionId);
                chatSession.setRepresentative(session);
                chatSession.setRepresentativeUsername(username);
                ChatEndpointListener.pendingSessions.remove(chatSession);
                session.getBasicRemote()
                        .sendObject(chatSession.getCreationMessage());
                session.getBasicRemote().sendObject(message);
            }

            ChatEndpointListener.sessions.put(session, chatSession);
            ChatEndpointListener.httpSessions.put(session, httpSession);
    ...
    {
    	log.entry();
        ChatSession c = ChatEndpointListener.sessions.get(session);
    ...
    {
    	log.entry(s);
        ChatSession c = ChatEndpointListener.sessions.get(s);
        Session other = this.getOtherSession(c, s);
        ChatEndpointListener.sessions.remove(s);
        HttpSession h = ChatEndpointListener.httpSessions.get(s);
        if(h != null)
            this.getSessionsFor(h).remove(s);
        if(c != null)
        {
            c.log(message);
            ChatEndpointListener.pendingSessions.remove(c);
            ChatEndpointListener.chatSessions.remove(c.getSessionId());
            try
            {
        ...
        if(other != null)
        {
            ChatEndpointListener.sessions.remove(other);
            h = ChatEndpointListener.httpSessions.get(other);
            if(h != null)
                this.getSessionsFor(h).remove(s);
            try
            {
            ...
            extends ServerEndpointConfig.Configurator
    {
                                    ...
                                    HandshakeResponse response)
        {
            ...
            config.getUserProperties().put(
                    ChatEndpointListener.HTTP_SESSION_PROPERTY, request.getHttpSession()
            );
            log.exit();
        }
    }
